...
export function getGearRatiosSum(engineMap: string[]) {
    ...
    gearIndices.forEach(gearIndice => {
        ...
        if (gearNumberPositions.length > 1) {
            gearRatiosSum += gearNumberPositions.reduce((acc, curr) => acc * curr.value, 1);
        }
...
export function getNumberPositions(engineMap: string[]) {
    ...
    engineMap.forEach((row, rowIndex) => {
        for (let y = 0; y < row.length; y++) {
            if (isNumber(row[y])) {
                const position: NumberPosition = {rowIndex, columnRange: [y,y], value: null, part: false, surroundingCoordinates: []};
                ...
                position.surroundingCoordinates = getSurroundingPositions(position, engineMap);
...
export function isPartNumber(position: NumberPosition, engineMap: string[]) {
    return position.surroundingCoordinates.some(([x,y]) => !isNumber(engineMap[x][y]) && engineMap[x][y] !== '.' && engineMap[x][y] !== undefined);
}

export function getSurroundingPositions(position: NumberPosition, engineMap: string[]) {
    const {rowIndex, columnRange: [start, end]} = position;
    ...
    if (rowIndex > 0) {
        surroundingCoordinates.push(...yCoordinates.map(y => ([rowIndex - 1, y] satisfies [number, number])));
    }
...
export function getNumberFromPosition(position: NumberPosition, engineMap: string[]) {
    const {rowIndex, columnRange: [start, end]} = position;
